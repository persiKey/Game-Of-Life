...
void SequentialTorusGOFAlgorithm::FillBorder(std::shared_ptr<IBorderCheckBoard> m_current_state)
{
    for(index_t i = 0; i < m_current_state->Length(); ++i)
    {
        m_current_state->BigSet(0,i, m_current_state->BigGet(m_current_state->Height(),i));
        m_current_state->BigSet(m_current_state->BigHeight() - 1,i, m_current_state->BigGet(1,i));
    }

    for(index_t i = 0; i < m_current_state->Height(); ++i)
    {
